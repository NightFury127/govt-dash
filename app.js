...
class AmendmentDashboard {
  constructor() {
    ...
    this.sampleAmendments = [
      {
        ...
        date: "2025-01-15",
        description:
          "Comprehensive digital privacy legislation requiring explicit consent for data collection and providing citizens with data deletion rights.",
        status: "Active",
        timeline_days: 30,
        total_feedback: 1247,
      ...
      },
      {
        ...
        date: "2025-01-28",
        description:
          "Reform to improve healthcare access in rural areas and reduce wait times for critical medical procedures.",
        status: "Active",
        timeline_days: 60,
        total_feedback: 892,
      ...
      },
      {
        ...
        date: "2025-02-10",
        description:
          "Comprehensive climate legislation with renewable energy targets and carbon reduction goals.",
        status: "Draft",
        timeline_days: 45,
        total_feedback: 634,
      ...
      },
      {
        ...
        date: "2025-02-15",
        description:
          "Modernize education infrastructure with technology integration and digital learning platforms.",
        status: "Active",
        timeline_days: 35,
        total_feedback: 756,
    ...
    this.sampleAnalytics = {
      ...
      summary:
        "Citizens generally support this privacy protection amendment, appreciating stronger data rights and consent requirements. However, some express concerns about potential business impact and implementation complexity. Overall sentiment remains positive with citizens viewing this as essential protection for digital rights.",
    };
  ...
  setupEventListeners() {
    ...
    const refreshAmendmentsBtn = document.getElementById(
      "refresh-amendments-btn"
    );
    if (refreshAmendmentsBtn) {
      refreshAmendmentsBtn.addEventListener("click", () =>
  ...
  updateDashboardStats() {
    ...
    const activeAmendments = this.amendments.filter(
      (a) => a.status === "Active"
    ).length;
    const totalFeedback = this.amendments.reduce(
  ...
  renderAmendments() {
    ...
    container.innerHTML = this.amendments
      .map(
        (amendment) => `
            <div class="amendment-card" data-amendment-id="${amendment.id}">
                <div class="amendment-header">
                    ...
                    <span class="amendment-date">${this.formatDate(
                      amendment.date
                    )}</span>
                </div>
                ...
                <div class="amendment-footer">
                    <span class="amendment-status ${amendment.status.toLowerCase()}">${
          amendment.status
        }</span>
                    <div class="amendment-actions">
                        <button class="btn btn--sm btn--secondary edit-amendment" data-id="${
                          amendment.id
                        }">Edit</button>
                        <button class="btn btn--sm btn--primary view-analytics" data-id="${
                          amendment.id
                        }">Analytics</button>
                        <button class="btn btn--sm btn--outline delete-amendment" data-id="${
                          amendment.id
                        }">Delete</button>
                    </div>
                </div>
  ...
  populateAmendmentForm(amendment) {
    ...
    document.getElementById("amendment-timeline").value =
      amendment.timeline_days || 30;
  }
  ...
  async showAmendmentAnalytics(amendmentId) {
    const amendment = this.amendments.find((a) => a.id === amendmentId);
    if (!amendment) return;

    // Send amendment name to backend API
    try {
      await fetch("/api/send-amendment", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "x-api-key": "sk-2b8e1e7c-4f7a-4e2a-9c1e-8d2e7b1a6f3c",
        },
        body: JSON.stringify({ amendmentName: amendment.name }),
      });
    } catch (err) {
      console.error("Failed to send amendment to friend:", err);
    }
  ...
  generateAmendmentAnalytics(amendment) {
    return {
      ...
      sentiment_breakdown: {
        positive: Math.round(
          (amendment.sentiment.positive / amendment.total_feedback) * 100
        ),
        negative: Math.round(
          (amendment.sentiment.negative / amendment.total_feedback) * 100
        ),
        neutral: Math.round(
          (amendment.sentiment.neutral / amendment.total_feedback) * 100
        ),
      },
      response_rate: "7.8%",
      average_sentiment: (
        amendment.sentiment.positive /
        (amendment.sentiment.positive + amendment.sentiment.negative)
      ).toFixed(2),
    };
  }
  ...
  createOverallSentimentChart() {
    ...
    const totalPositive = this.amendments.reduce(
      (sum, a) => sum + a.sentiment.positive,
      0
    );
    const totalNegative = this.amendments.reduce(
      (sum, a) => sum + a.sentiment.negative,
      0
    );
    const totalNeutral = this.amendments.reduce(
      (sum, a) => sum + a.sentiment.neutral,
      0
    );
